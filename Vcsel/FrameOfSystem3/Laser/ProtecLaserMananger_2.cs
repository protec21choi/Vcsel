...
{
    class ProtecLaserMananger_2
    {
        #region Enum
        public enum EN_SET_RESULT_2
        {
            OK,
            WORKING,
            FAIL,
        ...
        private Dictionary<int, LaserChannelParameter_2> m_dicLaserParam = new Dictionary<int, LaserChannelParameter_2>();

        private TickCounter_.TickCounter m_TickForTimeOut = new TickCounter_.TickCounter();

        private ProtecLaserController_2 m_ProtecLaser = ProtecLaserController_2.GetInstance();

        private ProtecLaserChannelCalibration_2 m_LaserCal = ProtecLaserChannelCalibration_2.GetInstance();
        ...
        #region SingleTone
        private ProtecLaserMananger_2() { }
        private static ProtecLaserMananger_2 _Instance = null;
        public static ProtecLaserMananger_2 GetInstance() 
        {
            if (_Instance == null)
            {
                _Instance = new ProtecLaserMananger_2();
            }
            return _Instance; 
        }
        #endregion
        ...
        public bool Init(int nChannelCount)
        {
            ...
            for (int nIndex = 0; nIndex < nChannelCount; ++nIndex)
            {
                if (false == m_dicLaserParam.ContainsKey(nIndex))
                {
                    ...
                    m_dicLaserParam.Add(nIndex, new LaserChannelParameter_2(PortIndex, ChannelIndexInPort));
                }
            }
        ...
        public EN_SET_RESULT_2 SetParameter(bool[] bEnable, double[] arPower, int[] arTime, double dSidePowerPercent, int[] nSideChIndex)
        {
            switch (m_nSeq)
            {
                case 0:
...
//                               }
                    ...
                    for (int nIndex = 0; nIndex < m_nChannelCount; nIndex++)
                    {
                        for (int nStep = 0; nStep < 5; nStep++)
                        {
                            ...
                            if (nSideChIndex.Contains(nIndex) && bEnable[nIndex])
                            {
                                ...
                                {
                                    return EN_SET_RESULT_2.CH_POWER_OVER;
                                }
                            }
                            m_dicLaserParam[nIndex].StepVoltage[nStep]
                                = m_LaserCal.GetProcessCalibrationChannelData(nIndex, EN_CALIBRATION_PROCESS_LIST_2.POWER_WATT_VOLT, m_dicLaserParam[nIndex].StepPower[nStep]);
            ...
            return EN_SET_RESULT_2.WORKING;
        }

        public EN_SET_RESULT_2 SetParameter(bool[] bEnable, double dPower, int nTime)
        {
            switch (m_nSeq)
            {
                case 0:
...
//                     }
        ...
        public EN_SET_RESULT_2 SetParameterIOMode(bool[] bEnable, double dTotalPower)
        {
            switch (m_nSeq)
            {
                case 0:
                ...
                case 1:
                    ...
                    for (int nIndex = 0; nIndex < m_nChannelCount; nIndex++)
                    {
                        if (m_dicLaserParam[nIndex].Enable)
                        {
                            ...
                            m_dicLaserParam[nIndex].VoltageIOMode = m_LaserCal.GetProcessCalibrationChannelData(nIndex, EN_CALIBRATION_PROCESS_LIST_2.POWER_WATT_VOLT, IOModePower);
                        }
                    }
                    m_nSeq++;
                    break;
        ...
        public EN_SET_RESULT_2 SetParameterTimeLimit(int nLimitSec_2)
        {
            switch (m_nSeq)
            {
                case 0:
                    {
                        var resultTimeLimit = m_ProtecLaser.SetIOModeTimeLimit(nLimitSec_2);
                        if (resultTimeLimit == ProtecLaserController_2.EN_RESULT_2.DONE)
                        {
                            m_nSeq++;
                            break;
                        }
                        if (resultTimeLimit == ProtecLaserController_2.EN_RESULT_2.FAIL)
                        {
                            m_nSeq = 0;
                            return EN_SET_RESULT_2.FAIL;
                        }
                        return EN_SET_RESULT_2.WORKING;
                    }
        ...
        public EN_SET_RESULT_2 SetParameterVolt(bool[] bEnable, double dVolt, int nTime)
        {
            switch (m_nSeq)
            {
                case 0:
...
//                    
        ...
        public EN_SET_RESULT_2 CheckShort(bool[] bEnable)
        {
            switch (m_nSeq)
            {
                case 0:
        ...
        private class LaserChannelParameter_2
        {
            public LaserChannelParameter_2(int PortIndex, int nChannelIndex)
            {
