...
using FrameOfSystem3.SubSequence.SubPickPlace.PickPlaceListMotionTouchForce;

using Motion_;

namespace FrameOfSystem3.SubSequence.SubPickPlace
{
    namespace PickPlaceListMotionTouchForce
    {
        public enum EN_AXIS
        {
			Z = 0,
    ...
    class SubCtrlPickPlaceListMotionTouchForce : ASubControl, IPickPlace
    {
        #region Variable
        ...
        #region <CONSTRUCTOR>
        public SubCtrlPickPlaceListMotionTouchForce(Task.RunningTaskEx tRunningTask)
            : base(tRunningTask)
        {
        }
        ...
        public MOTION_RESULT MoveContactPosition(double dSearchEndPos, double dSearchSpeed, int nSpeedRatio, bool bTouchUsed, double dTouchGap, int nPreDelay, int nPostDelay)
		{
            ...
            if (bTouchUsed)
            {
                ...
                return m_InstanceTask.MoveByListUntilTouch_Analog(nAxisZ, StepCount, m_arPos, arForceIndex, dblTouchThreshold, m_arSpeed, m_arRatio, m_arSpeedContent, nPostDelay, nPreDelay);
            }
            else
            {
