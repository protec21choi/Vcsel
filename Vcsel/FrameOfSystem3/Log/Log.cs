...
namespace FrameOfSystem3.Log
{
	...
	public class LogManager
    {
        ...
        private string[] m_arMappingCOMM            = null;
        ...
        private void MakeMappingArraies()
		{
            ...
            #region COMM
            m_arMappingCOMM = new string[Enum.GetValues(typeof(EN_COMMUNICATION_TYPE)).Length];

            foreach (EN_COMMUNICATION_TYPE enType in Enum.GetValues(typeof(EN_COMMUNICATION_TYPE)))
            {
                m_arMappingCOMM[(int)enType] = enType.ToString();
            }
            #endregion End - COMM
        }
        /// <summary>
        ...
        /// <summary>
        /// 2021.01.18 by wdw [ADD] Write the Communication log.
        /// </summary>
        public void WriteCommunicationLog(string strID, EN_COMMUNICATION_TYPE enType, string strData)
        {
            if (false == m_bInit) { return; }

            string strMessage = string.Format("{0} '{1}' 'COMM' '{2}' '{3}' '{4}'"
                , GetCurrentTime()
                , strID
                , m_arMappingCOMM[(int)enType]
                , strData
                , GetCurrentProcessRecipe());
